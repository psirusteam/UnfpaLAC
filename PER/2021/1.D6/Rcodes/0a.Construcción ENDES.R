...
#############################################################################################################################################
# #
# # Proceso: Construcción BAse ENDES Completa (Todos los modulos) 2021
# #                                                ------------- 
# #
# # Historia Archivo:
# #   Creation : 23/06/2022
...
#### Cargando los modulos de la encuesta ENDES
#
# ######
# ############# FILTRO - CREACION BASE
# 
# REC0111  = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1631/REC0111.SAV",to.data.frame = TRUE)
# REC21    = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1632/REC21.SAV", to.data.frame = TRUE)
# RE223132 = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1632/RE223132.SAV", to.data.frame = TRUE)
# RE516171 = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1635/RE516171.SAV",to.data.frame = TRUE)
# REC91    = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1631/REC91.SAV",to.data.frame = TRUE)
# RECH0    = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1629/RECH0.SAV",to.data.frame = TRUE)
# RECH1    = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1629/RECH1.SAV",to.data.frame = TRUE)
# RECH23   = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1630/RECH23.SAV",to.data.frame = TRUE)
# RECH4    = read.spss("PER/2021/1.D6/Data/Módulos/760-Modulo1629/RECH4.SAV",to.data.frame = TRUE)
# 
...
#     HHID, CASEID, V005, HV005, FEXM,V001,V012,V013, V021,V022,V023, V024, HV023, HV024, SPROVIN, SHPROVIN, SH15Y,SH15N,
...
#     QD333_2 ,QD333_3,QD333_4,QD333_5,QD333_6,S119,V155,S108N,S108Y,S108G,SH15N,
#     HV109,S111,HV110,SH13,V716,V717,V501,HV115,S119D,V130,V201,V310,V206,V207,V151,
...
# saveRDS(ENDESMEF, file = "PER/2021/1.D6/Data/ENDESMEF.rds")

## cargando la base de datos guardada

ENDESMEF <- readRDS(file = "PER/2021/1.D6/Data/ENDESMEF.rds")
...
##Sexo confirmando que solo sean seleaccionadas mujeres

table(ENDESMEF$HV104)
...
ENDESMEF$escolaridad <- ifelse(ENDESMEF$SH15N == "Preescolar", 0,
...
ENDESMEF$escolaridad[is.na(ENDESMEF$escolaridad)] <- 0
...
ENDESMEF$FEXM = ENDESMEF$V005 * 7845787 / sum(ENDESMEF$V005) ### averiguar  cuantas mujeres están es
...
ENDESMEF$union_1 = ifelse(ENDESMEF$V501 %in% c("Casada", "Conviviente"),
                          "Unida",
                          "No unida")
...
ENDESMEF$unida = ifelse(ENDESMEF$V501 %in% c("Casada", "Conviviente"),1,2)
...
ENDESMEF$Nec_ins_pf_E1 = ifelse(ENDESMEF$V624 == "Necesidad no satisfecha de espacio", 1, 0)
ENDESMEF$Nec_ins_pf_L1 = ifelse(ENDESMEF$V624 == "Necesidad no satisfecha de límite", 1, 0)
ENDESMEF$Nec_ins_pf_T1 = ifelse(ENDESMEF$Nec_ins_pf_E1 == 1 |
                                 ENDESMEF$Nec_ins_pf_L1 == 1, 1, 0)

## Necesidad satisfecha de planificacion familiar

ENDESMEF$Nec_sat_pf_E1 = ifelse(ENDESMEF$V624 == "Uso de espacio", 1, 0)
ENDESMEF$Nec_sat_pf_L1 = ifelse(ENDESMEF$V624 == "Uso de límite", 1, 0)
ENDESMEF$Nec_sat_pf_T1 = ifelse(ENDESMEF$Nec_sat_pf_E1 == 1 |
                                 ENDESMEF$Nec_sat_pf_L1 == 1, 1, 0)
...
ENDESMEF$Dem_tot_pf_E = ifelse(
  ENDESMEF$V626 %in% c(
    "Necesidad no satisfecha de espacio",
    "Uso de espacio",
    "Falla de espacio"
  ),
  1,
...
ENDESMEF$Dem_tot_pf_L = ifelse(
  ENDESMEF$V626 %in% c(
    "Necesidad no satisfecha de límite",
    "Uso de límite",
    "Falla de límite"
  ),
  1,
...
ENDESMEF$DemandaT = ENDESMEF$Nec_sat_pf_T + ENDESMEF$Nec_ins_pf_T
table(ENDESMEF$DemandaT)
...
## NI otra definición
table(ENDESMEF$Nec_ins_pf_T1, useNA = "a")
ENDESMEF$NI1  <- ifelse(ENDESMEF$Nec_ins_pf_T1 == 1, 1, 0)
table(ENDESMEF$NI1, useNA = "a")
ENDESMEF$NI1 <- ifelse(ENDESMEF$unida != 1, NA, ENDESMEF$NI1)
table(ENDESMEF$NI1, useNA = "a")
ENDESMEF$NI1[is.na(ENDESMEF$NI1)] <- 0
table(ENDESMEF$NI1, useNA = "a")
...
saveRDS(EndesMEF21, file = "PER/2021/1.D6/Data/EndesMEF21.rds")
...
ENDESMEF_Estima <- ENDESMEF %>%
  select(upm,estrato,FEXM,Cod_provi_full, Departamento,Provin_nom,
         Nec_ins_pf_T, Nec_sat_pf_T, Falla_met, DemandaT,
         area, anoest,etnia2, 
         edad, unida, discapacidad,
         D6, D6m, NI, NI1, V312,escolaridad)

colnames(ENDESMEF_Estima) <- c("upm","estrato", "fexp","mpio", "Depto", "Provincia",
                               "Nec_ins_pf_T","Nec_sat_pf_T", "Falla_met", "DemandaT",
                               "area","anoest","etnia","edad", "unida", "discapacidad",
                               "usametodo", "usamoderno", "nec_insat", "nec_insat2","tipodemetodo","escolaridad")

################

saveRDS(ENDESMEF_Estima,"PER/2021/1.D6/Data/ENDESMEF_Estima.rds")
saveRDS(ENDESMEF_Estima,"PER/2021/2.D6m/Data/ENDESMEF_Estima.rds")
saveRDS(ENDESMEF_Estima,"PER/2021/3.NI/Data/ENDESMEF_Estima.rds")
saveRDS(ENDESMEF_Estima,"PER/2021/4.D7/Data/ENDESMEF_Estima.rds")
