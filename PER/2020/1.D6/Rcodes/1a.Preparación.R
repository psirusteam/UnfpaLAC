...
####################################################
encuesta <- readRDS("PER/2020/1.D6/Data/ENdesMEF20.rds")
censo_mrp <- readRDS("PER/2020/1.D6/Data/censo_mrp.rds")
...
table(encuesta$mpio, useNA = "always")
...
table(encuesta$discap, useNA = "always")
labelled::generate_dictionary(encuesta %>%
                                select(discap))
...
### Endes: creando las variables de postestratificación:
...
encuesta_mrp <- encuesta %>%
  transmute(
    ...
    area = case_when(area == "Urbano" ~ "1",
                                 TRUE ~ "0"),
    
    edad = case_when(
                 edad %in% 0:14  ~  "1",# 5 a 14
    ...
    etnia = case_when(
                 ...
                 etnia == "Negro/ Moreno/ Zambo/ Mulato/Pueblo Afroperuano o afrodescendiente" 
                                          ~ "2", # afro negro mulato
                                     TRUE ~ "3" # Otro
    ),
    ...
    anoest = case_when(
      ...
      anoest %in% c(7:11)  ~ "3",# 7 - 12
      anoest  >  11        ~ "4",# mas de 12
                      TRUE ~ "Error",
    ),
    
    discapacidad = as.character(discap),
    
    unida = case_when(unida == 1 ~ "1",
                      unida == 2 ~ "2"
    ),
...
saveRDS(encuesta_mrp, file = "PER/2020/1.D6/Data/encuesta_mrp.rds")
saveRDS(encuesta_mrp, file = "PER/2020/2.D6m/Data/encuesta_mrp.rds")
saveRDS(encuesta_mrp, file = "PER/2020/3.NI/Data/encuesta_mrp.rds")
...
jpeg(filename = "PER/2020/1.D6/Output/plot_actualizacion_censo1.jpeg",
     width = 2000,
     height = 2000)
plot(censo_mrp$n, n1)
dev.off()
...
jpeg(filename = "PER/2020/1.D6/Output/plot_actualizacion_censo2.jpeg",
     width = 2000,
     height = 2000)
par(mfrow = c(2, 2))
hist(censo_mrp$n)
...
saveRDS(censo_mrp, "PER/2020/1.D6/Data/censo_mrp.rds")
saveRDS(censo_mrp, "PER/2020/2.D6m/Data/censo_mrp.rds")
saveRDS(censo_mrp, "PER/2020/3.NI/Data/censo_mrp.rds")
