...
fit_mrp_logit_D6  <- readRDS("MEX/2018/1.D6/Data/fit_mrp_logit.rds")
fit_mrp_logit_D6m <- readRDS("MEX/2018/2.D6m/Data/fit_mrp_logit.rds")
fit_mrp_logit_NI  <- readRDS("MEX/2018/3.NI/Data/fit_mrp_logit.rds")
...
byAgrega <-
  grep(
    pattern =  "^(n|usametodo|usamoderno|sexo|
              |upm|estrato|fexp)",
    x = names(censo_mrp),
    invert = TRUE,
    value = TRUE
...
saveRDS(poststrat_df %>%
          select(byAgrega,n,pobreza), 
        "MEX/2018/4.D7/Data/poststrat_df.RDS")
