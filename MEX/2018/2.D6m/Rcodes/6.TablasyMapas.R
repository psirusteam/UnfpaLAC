...
bynames <-
  grep(
    pattern =  "^(X|F|n|pobreza|ingreso|tasa_desocupacion|sexo|epred_mat|gk|crops.coverfraction|urban.coverfraction|stable_lights|dam|lp|.groups)",
...
bynames <- mutate(
  bynames,
  outPaht = paste0("MEX/2018/2.D6m/Output/pobreza_", paste0(V1, "_", V2),
                   ".pdf")
)
...
tmap_save(
  Mapa_lp,
  "MEX/2018/2.D6m/Output/Estados.pdf",
  width = 6920,
  height = 4080,
  asp = 0
)
...
openxlsx::write.xlsx(dat_df, file = "MEX/2018/2.D6m/Output/tablas.xlsx", 
                     overwrite = TRUE, )
