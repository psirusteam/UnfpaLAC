...
ENDS <- ENDS %>%
  transmute(
    ...
    etnia = case_when(etnia == 1        ~ "Indigena", # Indigena
                      etnia %in% c(3:5) ~ "Afrodescendiente", # afro negro mulato
                      TRUE ~ "Otro"  # Otro
    ),
...
D6_area <- D6_area[c(1,3,4)]
D6_area$D6_cv <- D6_area$D6_cv * 100
D6_area$D6_cv <- round(D6_area$D6_cv, digits = 1)
colnames(D6_area) <- c("Departamento", "D6", "CV")
D6_area
...
nivel_educativo<-c("Sin educación",
                   "Primaria",
                   "Secundaria",
                   "Superior")
...
D6_anoest <- D6_anoest[c(1,3,4)]
D6_anoest$D6_cv <- D6_anoest$D6_cv * 100
D6_anoest$D6_cv <- round(D6_anoest$D6_cv, digits = 1)
colnames(D6_anoest) <- c("Nivel educativo", "D6", "CV")
D6_anoest
...
###### Creando archivo Tabla

wb<-createWorkbook()


hstyle<- createStyle(halign = "center",
                     valign = "center",
                     fontSize = 9,
                     fontName = "corbel",
                     border = c("Top","Bottom"),
...
#### Creando hojas en el archivo

addWorksheet(wb,"depto",gridLines = F)
addWorksheet(wb,"area",gridLines = F)
addWorksheet(wb,"edad",gridLines = F)
addWorksheet(wb,"sexo",gridLines = F)
addWorksheet(wb,"unida",gridLines = F)
addWorksheet(wb,"anoest",gridLines = F)
addWorksheet(wb,"etnia",gridLines = F)
addWorksheet(wb,"censo_mrp",gridLines = F)
addWorksheet(wb,"area_mrp",gridLines = F)
addWorksheet(wb,"etnia_mrp",gridLines = F)
addWorksheet(wb,"edad_mrp",gridLines = F)
addWorksheet(wb,"anoest_mrp",gridLines = F)
addWorksheet(wb,"sexo_mrp",gridLines = F)
addWorksheet(wb,"unida_mrp",gridLines = F)
addWorksheet(wb,"tabla",gridLines = F)
addWorksheet(wb,"tasa_desocupacion",gridLines = F)


writeData(wb,"depto",depto, headerStyle = hstyle )
writeData(wb,"area",area, headerStyle = hstyle )
writeData(wb,"edad",edad, headerStyle = hstyle )
writeData(wb,"sexo",sexo, headerStyle = hstyle )
writeData(wb,"unida",unida, headerStyle = hstyle )
writeData(wb,"anoest",anoest, headerStyle = hstyle )
writeData(wb,"etnia",etnia, headerStyle = hstyle )
writeData(wb,"censo_mrp",censo_mrp, headerStyle = hstyle )
writeData(wb,"area_mrp",area_mrp, headerStyle = hstyle )
writeData(wb,"etnia_mrp",etnia_mrp, headerStyle = hstyle )
writeData(wb,"edad_mrp",edad_mrp, headerStyle = hstyle )
writeData(wb,"sexo_mrp",sexo_mrp, headerStyle = hstyle )
writeData(wb,"anoest_mrp",anoest_mrp, headerStyle = hstyle )
writeData(wb,"unida_mrp",unida_mrp, headerStyle = hstyle )
writeData(wb,"tabla",tabla, headerStyle = hstyle )
writeData(wb,"tasa_desocupacion",tasa_desocupacion, headerStyle = hstyle )

addStyle(wb,"depto",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"area",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"edad",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"sexo",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"unida",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"anoest",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"etnia",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"censo_mrp",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"etnia_mrp",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"unida_mrp",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"area_mrp",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"anoest_mrp",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"edad_mrp",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"sexo_mrp",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"tabla",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)
addStyle(wb,"tasa_desocupacion",cstyle, rows = 2:2000, cols = 1:150,
         gridExpand = T)


saveWorkbook(wb, "COL/2015/1.D6/Output/Tablas/Estimaciones_directas_D6.xlsx",
             overwrite = T
)
