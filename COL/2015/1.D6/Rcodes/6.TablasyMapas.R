...
bynames <- mutate(
  bynames,
  outPaht = paste0("COL/2015/1.D6/Output/pobreza_", paste0(V1, "_", V2),
                   ".pdf")
)
...
cod_shape <- ShapeSAE %>% 
  as.data.frame() %>% select(mpio, nombre)
...
tmap_save(
  Mapa_lp,
  "COL/2015/1.D6/Output/Estados.pdf",
  width = 6920,
  height = 4080,
  asp = 0
)
...
openxlsx::write.xlsx(dat_df, file = "COL/2015/1.D6/Output/tablas.xlsx", 
                     overwrite = TRUE, )
