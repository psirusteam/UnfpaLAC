...
poststrat_df <- readRDS("ECU/2018/2.D6m/Data/poststrat_df.RDS") %>% 
  filter(anoest != "99")
...
mrp_estimate = map(1:nrow(byAgrega),
             ~poststrat_df %>% 
               group_by_at(vars("mpio", byAgrega[.x,])) %>%
...
openxlsx::write.xlsx(tablas, file = "ECU/2018/2.D6m/Output/tablas_ECM.xlsx", 
                     overwrite = TRUE)
