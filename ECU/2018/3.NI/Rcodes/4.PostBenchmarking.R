...
tasa_desocupados <- readRDS("ECU/2018/3.NI/Data/tasa_desocupacion.rds")
fit <- readRDS("ECU/2018/3.NI/Data/fit_mrp_logit.rds")

########Eliminando los NA's

#### encuesta
table(encuesta_mrp$necesInst, useNA = "a")
...
# Calculo de uso de necesidades insatisfechas.  -------------------------------------------

## definiendo necesidades insatisfechas como pobreza
...
jpeg(file = "ECU/2018/3.NI/Output/Plot_Bench_Pobreza.jpeg")
hist(poststrat_df$gk)
dev.off()

saveRDS(poststrat_df %>%
        select(!matches("_\\d{,2}$")), 
        "ECU/2018/3.NI/Data/poststrat_df.RDS")

names(encuesta_mrp)

saveRDS(encuesta_mrp, "ECU/2018/3.NI/Data/encuesta_mrp.RDS")
